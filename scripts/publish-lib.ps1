...
$script:DefaultRemoteReleasesDir = '/home/ambrus/www/aml/diagnostic.aml.one/downloads'
$script:DefaultSshHost = 'ambrus@frankfurt.aml.one'
$script:DefaultSshPort = 717
$script:DownloadsUrl = 'https://aml.one/diagnostic-ota'
...
$script:ArtifactPatterns = @{
    ...
    diagnosticLinux      = '^diagnostic-linux-x64-v\d+\.\d+\.\d+-\d{6}\.deb$'
}
...
function Get-PublishSshTarget {
    param(
        [string]$SshTarget = $env:AML_FRANKFURT_SSH,
    ...
    if ($SshPort -le 0) {
        $SshPort = if ($env:AML_FRANKFURT_SSH_PORT -and [int]$env:AML_FRANKFURT_SSH_PORT -gt 0) {
            [int]$env:AML_FRANKFURT_SSH_PORT
        } else {
...
function Read-ReleaseFromFilename {
    ...
    if (-not $script:ArtifactPatterns.ContainsKey($Platform)) {
        throw "Unknown platform key '$Platform'. Diagnostic publishes only: $($script:PlatformKeys -join ', ')"
    }
...
function Merge-RemoteReleaseManifest {
    ...
    $remotePy = '/tmp/diagnostic-release-manifest.py'
    $remoteEntry = '/tmp/diagnostic-release-entry.json'
